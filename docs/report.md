...
## Формализация вербальной постановки
...
Теперь Отдел обладает двумя наборами ключей (в каждом один атрибут). Связь 1:N сформирована при помощи "добавления" атрибута ```Номер отдела``` к уже таблице Сотрудник.
